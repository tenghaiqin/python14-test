...
log = Log()
test_data = Do_excel(path.case_path,'recharge').read_data('RechargeCase')
...
class TestRegister(unittest.TestCase):
    ...
    def setUp(self):
        try:
            self.excel=Do_excel(path.case_path,'recharge')
    ...
    def test_succcess(self,case):
        ...
        if case['Sql'] !=None:
            ...
            log.info('充值之前的金额为{}'.format(before_amount))
        ...
        try:
            if case['Sql']!=None:
                ...
                log.info('充值之后的金额为{}'.format(after_amount))

                recharge_amount = int(param['amount'])
                log.info('充值金额{}'.format(recharge_amount))
                except_amount = before_amount + recharge_amount
