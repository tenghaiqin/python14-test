...
from API_6.common.pubilc.my_log import MyLog
my_log=MyLog()
...
class DoExcel:
    ...
    def read_data(self,section):#添加一个片段名，根据指定的片段名来执行具体用例--4
        '''读取excel中所有的测试数据，有返回值'''
        try:
            wb=load_workbook(self.file_name)
            sheet=wb[self.sheet_name]
            Case_id=ReadConfig(project_path.conf_path).get_str(section,'Case_id')
